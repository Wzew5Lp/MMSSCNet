...
subsequent encoder_decoder based architectures
"""
...
class TransformerDecoderLayer(TransformerDecoderLayerBase):
    ...
    def _forward(
        ...
        """A naive forward pass for encoder_decoder decoder.
...
class TransformerDecoder(TransformerDecoderBase):
    ...
    def init_state(self, src=None, memory_bank=None, enc_hidden=None):
        super(TransformerDecoder, self).init_state(None, None, None)
    ...
    def forward(self, tgt_emb, memory_bank, src_pad_mask=None, tgt_pad_mask=None, step=None, **kwargs):
        ...
        tgt_words = tgt_emb[:, :, 0].transpose(0, 1)

        emb = self.embeddings(tgt_emb, step=step)
        assert emb.dim() == 3  # len x batch x embedding_dim
        ...
        with_align = kwargs.pop("with_align", False)
        assert not with_align, "TransformerDecoder does not support align"
        hiddens = []
