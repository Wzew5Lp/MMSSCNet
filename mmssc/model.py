...
from .encoder_decoder import TransformerDecoder, Embeddings
...
class Encoder(nn.Module):
    ...
    #             features, hiddens = self.swin_forward(self.encoder_decoder, x)
    #         else:
    #             features, hiddens = self.encoder_decoder(x)
    #     else:
    #         raise NotImplemented
    ...
    def forward(self, x, refs=None):
        if self.model_type in ['resnet', 'efficientnet']:
            ...
            if 'patch' in self.model_name:
                ...
                # print(self.encoder_decoder(x).shape)
...
class TransformerDecoderAR(TransformerDecoderBase):
    ...
    # adapted from onmt.decoders.encoder_decoder
