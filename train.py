...
from mmssc.dataset import TrainDataset, AuxTrainDataset, bms_collate
from mmssc.model import Encoder, Decoder
from mmssc.loss import Criterion
from mmssc.utils import seed_torch, save_args, init_summary_writer, LossMeter, AverageMeter, asMinutes, timeSince, \
    print_rank_0, format_df
from mmssc.Chemical_library import convert_graph_to_smiles, postprocess_smiles
from mmssc.evaluate import SmilesEvaluator
from mmssc.tokenizer import get_tokenizer
...
def train_loop(args, train_df, valid_df, aux_df, tokenizer, save_path):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for name in ['acc_smiles', 'graph_smiles', 'canon_smiles']:
            if name in scores:
                score = scores[name]
                break
...
def inference(args, data_df, tokenizer, encoder=None, decoder=None, save_path=None, split='test'):
    ...
    if 'SMILES' in data_df.columns:
        ...
        if 'acc_SMILES' in pred_df.columns:
            acc_scores = evaluator.evaluate(pred_df['acc_SMILES'])
            scores['acc_smiles'] = acc_scores['canon_smiles']
            scores['acc_graph'] = acc_scores['graph']
...
def main():
    ...
    if args.local_rank != -1:
        n_gpus = 1
        dist.init_process_group("nccl", world_size=n_gpus, rank=args.local_rank, init_method='env://', timeout=datetime.timedelta(0, 14400))
        torch.cuda.set_device(args.local_rank)
        torch.backends.cudnn.benchmark = True
